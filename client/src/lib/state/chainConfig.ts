...
const { publicClient, webSocketPublicClient } = configureChains(
	...
	[
		...
		alchemyProvider({ apiKey: 'pvNYFwyAKEEyZqsPdStj8B3mJqsUEnFM' })
	]
);
...
export const ethClient = writable(new EthereumClient(wagmiConfig, supportedChains));
...
export const signer = derived([account, network], ([$account, $network], set) => {
	if (!$network?.chain?.id || !$account) return;

	const noSignerError = (e?: any) => console.error('Signer fetching failed', e);
