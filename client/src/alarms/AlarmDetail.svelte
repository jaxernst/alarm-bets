<script lang="ts">
	...
	$: if (
		...
		correctedAlarmTime = correctAlarmTime(
			...
			Number(account === $alarm.player1 ? $alarm.player1Timezone : $alarm.player2Timezone)
		);
	}
	...
	const submitConfirmationTransaction = async () => {
		...
		submitPending = true;
		try {
			const txResult = await transactions.addTransaction(submitConfirmation(alarmAddress));
			if (!txResult.error) {
...
<div class="flex h-full flex-col">
	<div class="flex flex-grow flex-col gap-1">
		<div class="custom-grid gap-4">
			...
			>
				<ClockDisplay
					overrideTime={timeString(correctedAlarmTime ?? Number($alarm.alarmTime))}
						...
						>
							...
							<MenuItem>
								<button
									...
									on:click={() => ($showEndAlarmModal = !$showEndAlarmModal)}>End Alarm</button
								>
							</MenuItem>
						</MenuItems>
					{/if}
		...
		{#if Number($alarm.alarmTime) !== correctedAlarmTime}
			<div class="pb-1 text-center text-xs text-red-600">
				...
				}`}) differs from your preset alarm timezone. (Displaying adjusted alarm times)
			</div>
		{/if}
		...
		<div class="bg-transparent-grey rounded-lg p-2 text-sm">
			<button class="w-full" on:click={() => (showAlarmInfo = !showAlarmInfo)}>
				<div class="flex justify-between font-bold text-zinc-400">
					<div>Alarm Info</div>
					<div class={`text-[8px] ${rotatedDropdown()} transition duration-100`}>&#9660;</div>
				</div>
			</button>
			{#if showAlarmInfo}
				<div transition:slide={{ easing: expoOut, duration: 600 }}>
					<div class="flex justify-between gap-2">
						...
						<span
							><span class="h-3 w-3 text-cyan-600">{Number($alarm.submissionWindow) / MINUTE}</span>
							minutes</span
						>
					</div>
					...
					<div class="flex justify-between gap-2">
						...
						<span
							><span class="h-3 w-3 text-cyan-600">{formatEther($alarm.missedAlarmPenalty)}</span> eth</span
						>
					</div>
					<div class="flex justify-between gap-2">
						<span class="text-zinc-400">initial deposit:</span>
						<span
							><span class="h-3 w-3 text-cyan-600">{formatEther($alarm.betAmount)}</span> eth</span
						>
					</div>
				</div>
			{/if}
	...
	{#if $alarm.status === AlarmStatus.ACTIVE}
		<div class="bg-highlight-transparent-grey mt-1 flex justify-center rounded-lg">
			{#if submitPending}
				<div class="p-3">
					<DiamondSpinner size={'30'} color={'white'} />
				</div>
			{:else}
				<button
					...
					>{confirmationSubmitted ? 'Wakeup Submitted' : 'Confirm Wakeup'}</button
				>
			{/if}
		</div>
	{/if}
