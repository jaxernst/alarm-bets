<script lang="ts">
	...
	const joinAlarm = async () => {
		...
		try {
			...
			const txResult = await transactions.addTransaction(startAlarm(targetAlarm, tzOffset));
			if (!txResult.error) {
				toast.push(`Successfully joined alarm with ${shorthandAddress(otherPlayer)}!`);
				$activeTab = 'alarms';
			} else {
	...
	const searchForAlarm = async () => {
		...
		try {
			...
			searchedAlarm = {
				...
				player1Timezone: Number(await getPlayerTimezone(alarmAddr, constants.player1))
			};
		} catch {
...
<div class="flex h-full flex-col gap-2 px-2 pt-2">
	<div class=" flex h-[30px] items-center gap-2 rounded-xl px-2">
		...
		{:else}
			<button
				class={`rounded-xl px-2 text-sm text-cyan-400 ${!account ? 'opacity-50' : ''}`}
			...
			<div class="flex justify-center">
                ...
                "
					...
					<div class="flex justify-between gap-2">
						...
						<span
							><span class="h-3 w-3 text-cyan-600">{shorthandAddress(searchedAlarm.player1)}</span
							></span
						>
					</div>
					<div class="flex justify-between gap-2">
					...
					<div class="flex justify-between gap-2">
						...
						<span
							><span class="h-3 w-3 text-cyan-600">{formatEther(searchedAlarm.betAmount)}</span> eth</span
						>
					</div>
				</div>
			</div>
